...
def build_profile(g: Graph, manifest: dict) -> dict:
    ...
    # profile.json is the HCMO API inventory, not an inventory of every external
    # vocabulary term referenced by an axiom.
    local_root = manifest["ontology_iri"]

    def local_entries(rdf_type):
        return [entry(s) for s in terms(g, rdf_type) if str(s).startswith(local_root)]

    classes = local_entries(OWL.Class)
    obj_props = local_entries(OWL.ObjectProperty)
    dt_props = local_entries(OWL.DatatypeProperty)
    ann_props = local_entries(OWL.AnnotationProperty)
    return {
...
def main() -> int:
    ...
    # The compact RDF/XML serializer preserves RDF collection assertions used
    # by owl:unionOf; rdflib's pretty-xml serializer can drop list metadata.
    owl_xml = canon.serialize(format="xml")
    owl_marker = "<!-- DO NOT EDIT — GENERATED by tooling/build.py from ontology/modules/*.ttl -->\n"
    if owl_xml.startswith("<?xml"):
        declaration, rest = owl_xml.split("\n", 1)
        owl_xml = declaration + "\n" + owl_marker + rest
    else:
        owl_xml = owl_marker + owl_xml
    write_text(ROOT / dist["merged_owl"], owl_xml)
