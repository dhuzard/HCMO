...
| SemTS-derived structure | `semts:generated`, `semts:hasDimension` | `needs evidence`; HCMO uses the SemTS 1.2.0 ontology version IRI (`.../ontology/120#`) as a term namespace, while SemTS declares its entities in the unversioned `.../ontology#` namespace. Canonical IRIs and the semantic fit of knowledge-generation `generated` and time-series `segmentDimension` must be reviewed before these references count as SemTS reuse. |
