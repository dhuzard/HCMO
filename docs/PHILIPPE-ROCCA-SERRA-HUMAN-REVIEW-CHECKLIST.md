...
| A06 implementation | Cyril Gilbert (`https://orcid.org/0009-0008-2489-8106`) | 2026-07-30 | Working pass complete; defer semantic changes requiring evidence | Reviewed all 29 active local classes and 13 directly used external class anchors. Recorded 24 local `keep` and 5 `needs evidence` decisions after separating monitoring from installation and pinning the source-faithful upper anchors; checked labels, definitions, parents, restrictions, inferred hierarchy, provenance, CQ relevance, external versions, and generated-profile agreement. | `docs/CLASS-AUDIT-WORKING-NOTES.md`; `tooling/class_audit.py --catalog` | `docs/philippe-followup` |
...
| C01-01 | Cyril Gilbert (`https://orcid.org/0009-0008-2489-8106`) | 2026-07-22 | Defer; needs evidence | The current inverse makes every monitoring relation an installation relation and conversely. Existing examples assert both directions and do not cover remote or portable sensors. Domain evidence and a CQ are required before retaining, removing, or replacing the inverse. No axiom changed. | `docs/PROPERTY-AUDIT-WORKING-NOTES.md`; `tooling/property_audit.py` | `cyril/property-audit` |
| C01 coverage | Cyril Gilbert (`https://orcid.org/0009-0008-2489-8106`) | 2026-07-22 | Defer semantic changes; working pass complete | Reviewed all 81 active local properties, directly used external property groups, and all 49 deprecated compatibility properties. Stable terms are marked `keep`; unresolved semantics are explicitly marked `needs evidence`. Automated checks cover metadata, migration targets, priority domain/range effects, inverse effects, a parent-property effect, and a missing SOSA shortcut entailment. No axiom changed and the C03 final inventory schema remains pending. | `docs/PROPERTY-AUDIT-WORKING-NOTES.md`; `tooling/property_audit.py --catalog` | `cyril/property-audit` |
...
| C04 implementation | Cyril Gilbert (`https://orcid.org/0009-0008-2489-8106`) | 2026-07-22 | Implement accepted contract; ready for review | Documented the ontology/data/entailment boundary, supplied the canonical ontology graph to pySHACL with RDFS inference, added an inferred-target negative fixture, and made every indexed CQ assert an expected row count over the ontology plus positive examples. No ontology axiom or shape changed. | `docs/MODEL.md`; `docs/ARCHITECTURE.md`; `examples/abox-inferred-invalid.ttl`; `queries/competency_questions.yaml`; `tooling/validate.py` | `cyril/c04-entailment-contract` |
