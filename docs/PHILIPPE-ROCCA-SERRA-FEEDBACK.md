...
| Singular/plural duplicates (`hasDimension(s)`, `hasSensor(s)`) | Enclosure dimensions use local `hasDimensions` for one enclosure-dimensions record. The unrelated SemTS pattern now uses canonical `segmentDimension` only from a time-series segment to a data dimension; the invalid `hasDimension` reference was removed. Sensor monitoring uses `monitoredBy`; physical cage installation uses the separate, non-inverse `installedIn` relation. The plural UNKNOWN relation was removed. | core, obs, and tech modules |
